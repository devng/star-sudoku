...
import com.devng.starsudoku.*;
import com.devng.starsudoku.GV.NumDistributuon;
import com.devng.starsudoku.GV.NumberEntry;
import com.devng.starsudoku.io.Export;
import com.devng.starsudoku.io.ExportPDF;
import com.devng.starsudoku.io.LoadSaver;
...
//                            guiGrid.paintPause();
    ...
    private Image loadImage(String imageName) {
        try {
            ...
            Image im = ImageIO.read(new BufferedInputStream(
                    classloader.getResourceAsStream(GV.IMG_FOLDER +
                            imageName)));
            return im;
        } catch (Exception e) {
            e.printStackTrace(System.out);
    ...
    private void setAboutPanel() {
        ...
        about.add(new HTMLLink(GV.WEB_PAGE, false));
    ...
    private void setDiffLabelVal(int diff) {
        switch (diff) {
            case GV.DIFF_DESIGN:
                jTextDiff.setText("User design");
                break;
            case GV.DIFF_EASY:
                jTextDiff.setText("Easy");
                break;
            case GV.DIFF_NORMAL:
                jTextDiff.setText("Normal");
                break;
            case GV.DIFF_HARD:
                jTextDiff.setText("Hard");
                break;
            default:
    ...
    private void doSolve() {
        ...
        boolean b = askSureQuestion("Solve the grid automatically?");
    ...
    private boolean askSureQuestion(String sureFor) {
        if (hasTheGridBeenChanged) {
            ...
            sb.append("Do you really what to ");
            sb.append(sureFor);
